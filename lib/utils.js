...
import { cloneDeep, compact, every, find, findLast, first, flatMap, flattenDeep, isEqual, groupBy, last, maxBy, orderBy, reduce, size, some, sortBy, uniq, uniqBy } from 'lodash-es';
...
const filterTrips = timetable => {
  ...
  if (timetable.start_timestamp !== '' && timetable.start_timestamp !== null && timetable.start_timestamp !== undefined) {
    filteredTrips = filteredTrips.filter(trip => trip.stoptimes[0].arrival_timestamp >= timetable.start_timestamp);
  }
  ...
  if (timetable.end_timestamp !== '' && timetable.end_timestamp !== null && timetable.end_timestamp !== undefined) {
    filteredTrips = filteredTrips.filter(trip => trip.stoptimes[0].arrival_timestamp < timetable.end_timestamp);
  }
