...
function filterCalendarDates(dates, startDate, endDate) {
  ...
  return _.filter(dates, (date) => {
    return moment(date, 'YYYYMMDD').isBetween(start, end);
  });
}
...
exports.generateFilename = function generateFilename(agencyKey, timetable, cb) {
  ...
  if (timetable.timetable_file_name) {
    cb(null, formatFileName(timetable.timetable_file_name));
...
exports.generateHTML = function generateHTML(agencyKey, timetableId, options, cb) {
  ...
  gtfs.getTimetable(agencyKey, timetableId, (e, timetables) => {
    if (e) return cb(e);
    ...
    gtfs.getCalendars(agencyKey, startDate, endDate, timetable.monday, timetable.tuesday, timetable.wednesday, timetable.thursday, timetable.friday, timetable.saturday, timetable.sunday, (e, calendars) => {
      if (e) return cb(e);
          ...
          async.map(trips, (item, cb) => {
            ...
            gtfs.getStoptimesByTrip(agencyKey, trip.trip_id, (e, stoptimes) => {
              ...
              if (options.showOnlyTimepoint) {
                trip.stoptimes = _.filter(stoptimes, (stoptime) => {
