...
function getSpecialDates(serviceIds, timetable, cb) {
  gtfs.getCalendarDatesByService(serviceIds, (err, results) => {
...
exports.generateHTML = (agencyKey, timetableId, options, cb) => {
  ...
  async.series([
    ...
    (cb) => {
      // Lookup shapes
      gtfs.getShapesByRoute(agencyKey, timetable.route_id, timetable.direction_id, serviceIds, (err, results) => {
        if (err) return cb(err);
        ...
        shapes = _.map(results, (shape) => _.map(shape, (p) => [p.shape_pt_lat, p.shape_pt_lon]));
        return cb();
      });
    }
  ], (err) => {
