...
function formatCalendars(calendars) {
  return calendars.map(item => {
    ...
    calendar.dayList = formatDays(calendar);
    return calendar;
  });
}
...
function getTemplateFile(options) {
  if (options.templatePath !== undefined) {
    return options.templatePath;
  } else if (options.noHead === true) {
...
exports.generateHTML = (agencyKey, timetablePage, options) => {
  ...
  return Promise.all(timetablePage.timetables.map((timetable) => {
    ...
    .then(() => {
      ...
      timetable.dayList = formatDays(timetable);
    })
    ...
    .then(timetableStopOrders => {
      ...
      return Promise.all(timetable.trips.map(item => {
        ...
        trip.dayList = formatDays(trip.calendar);
  ...
  .then(() => {
    // Summarize Timetables in Timetable Page
  ...
  .then(() => {
    ...
    return pug.renderFile(getTemplateFile(options), templateVars);
  })
  .then(html => {
    if (options.beautify) {
