...
const filterTrips = (trips, timetable) => {
  ...
  if (timetable.start_timestamp !== '' && timetable.start_timestamp !== undefined) {
    filteredTrips = filteredTrips.filter(trip => {
      return trip.stoptimes[0].arrival_timestamp >= timetable.start_timestamp;
    });
  }

  if (timetable.end_timestamp !== '' && timetable.end_timestamp !== undefined) {
    filteredTrips = filteredTrips.filter(trip => {
...
const getTripsFromTimetable = async (timetable, calendars, config) => {
  ...
  await Promise.all(trips.map(async trip => {
    ...
    if (formattedTrip.stoptimes.length === 0) {
      config.logWarning(`No stoptimes found for agency_key=${timetable.agency_key}, trip_id=${formattedTrip.trip_id}, route_id=${timetable.route_ids.join('_')}, timetable_id=${timetable.timetable_id}`);
    }
