...
exports.generateHTML = (agencyKey, timetablePage, options) => {
  ...
  return Promise.all(timetablePage.timetables.map((timetable) => {
    return gtfs.getCalendars(agencyKey, timetable.start_date, timetable.end_date, timetable.monday, timetable.tuesday, timetable.wednesday, timetable.thursday, timetable.friday, timetable.saturday, timetable.sunday)
    .then(calendars => {
      if (!calendars.length) {
    ...
    .then(() => {
      processStops(agencyKey, timetable, options);
    })
    .then(() => {
    ...
    .then(shapes => {
      if (shapes) {
        timetable.shapes = _.map(shapes, shape => _.map(shape, p => [p.shape_pt_lat, p.shape_pt_lon]));
      } else {
