...
function filterAndSortCalendarDates(calendarDates, startDate = '19000000', endDate = '21000000') {
  if (!calendarDates) {
    return [];
  }
...
const getStoptimesByTrip = async (trip, config) => {
  ...
  const deduplicatedStoptimes = _.filter(stoptimes, (stoptime, idx) => {
    if (idx === 0 || stoptime.arrival_time === '') {
      return true;
    } else if (stoptime.arrival_time !== stoptimes[idx - 1].departure_time) {
      return true;
    }
    return false;
  });
...
exports.formatTimetables = async (timetables, config) => {
  return Promise.all(timetables.map(async timetable => {
    ...
    Object.assign(timetable, {
      ...
      serviceIds: _.uniq([...calendarServiceIds, ...calendarDatesServiceIds]),
      dayList: formatters.formatDays(timetable)
    });
