...
const getTripsWithSameBlock = async (trip, timetable) => {
  ...
  await Promise.all(trips.map(async blockTrip => {
    ...
    blockTrip.firstStoptimeSeconds = formatters.timeToSeconds(blockTrip.firstStoptime.departure_time);
  }));

  return _.sortBy(trips, 'firstStoptimeSeconds');
}
...
const addTripContinuation = async (trip, timetable) => {
  ...
  const soonestTrip = _.find(blockTrips, blockTrip => {
    return blockTrip.firstStoptimeSeconds >= lastStoptimeSeconds;
  });
  ...
  trip.continues_as_route = soonestTrip;
};
