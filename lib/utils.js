...
const addStoptimesToTrip = async (trip, timetable) => {
  let stopIndex = 0;
...
const processStops = async timetable => {
  ...
  await Promise.all(timetable.orderedTrips.map(trip => {
    ...
    return addStoptimesToTrip(trip, timetable);
  }));
...
exports.generateHTML = async (timetablePage, config) => {
  ...
  await Promise.all(timetablePage.timetables.map(async timetable => {
    ...
    timetable.calendars = await gtfs.getCalendars(calendarQuery);
    ...
    const formattedTrips = await Promise.all(trips.map(async item => {
      ...
      trip.stoptimes = (config.showOnlyTimepoint === true) ? filterStoptimes(stoptimes) : stoptimes;
...
exports.generateOverviewHTML = async (agencyKey, timetablePages, config) => {
  ...
  let html = await pug.renderFile(fileUtils.getOverviewTemplateFile(config), templateVars);
  if (config.beautify === true) {
    html = await beautify(html, {indent_size: 2});
  }
  return html;
