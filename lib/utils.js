...
const getCalendarDates = async timetable => {
  ...
  const start = formatters.fromGTFSDate(timetable.start_date);
  const end = formatters.fromGTFSDate(timetable.end_date);
...
const getTripsFromTimetable = async (timetable, calendars, config) => {
  ...
  await Promise.all(trips.map(async trip => {
    ...
    if (formattedTrip.stoptimes.length === 0) {
      if (config.showOnlyTimepoint === true) {
        logUtils.warn(`No stoptimes which are timepoints found for agency_key=${timetable.agency_key}, trip_id=${formattedTrip.trip_id}`);
      } else {
        logUtils.warn(`No stoptimes found for agency_key=${timetable.agency_key}, trip_id=${formattedTrip.trip_id}`);
      }
    }
...
/*
 * Format timetable pages for display.
 */
