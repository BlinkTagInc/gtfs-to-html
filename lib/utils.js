...
function processStops(agencyKey, timetable, options, cb) {
  ...
  gtfs.getStops(agencyKey, _.map(timetable.stops, 'stop_id'), (err, stopDatas) => {
    if (err) return cb(err);
...
function processStopsForMap(timetable, cb) {
  timetable.stopsForMap = timetable.stops.map(stop => _.pick(stop, ['stop_name', 'stop_times', 'stop_lat', 'stop_lon']));
  cb();
}
...
exports.zipFolder = (exportPath, cb) => {
