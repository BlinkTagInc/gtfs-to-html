...
const getStops = async (timetable, config) => {
  ...
  const orderedStops = await Promise.all(orderedStopIds.map(async (stopId, index) => {
    const stops = await gtfs.getStops({ stop_id: stopId });
  ...
  if (timetable.showStopCity) {
    const stopAttributes = await gtfs.getStopAttributes({ stop_id: orderedStopIds });
  ...
  const formattedStops = formatters.formatStops(orderedStops, timetable, config);
...
exports.getTimetablePages = async config => {
  const timetables = formatters.mergeTimetablesWithSameId(await gtfs.getTimetables());
...
exports.generateOverviewHTML = async (timetablePages, config) => {
  ...
  // Sort timetables for display, first numerically then alphaetically
  const sortedTimetablePages = sortBy(timetablePages, [
    timetablePage => {
      if (timetablePage.consolidatedTimetables.length > 0 &&  timetablePage.consolidatedTimetables[0].routes.length > 0) {
        return parseInt(timetablePage.consolidatedTimetables[0].routes[0].route_short_name, 10);
      }
    },
    timetablePage => {
      if (timetablePage.consolidatedTimetables.length > 0 &&  timetablePage.consolidatedTimetables[0].routes.length > 0) {
