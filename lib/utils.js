...
exports.generateHTML = (agencyKey, timetablePage, options, cb) => {
  ...
  async.each(timetablePage.timetables, (timetable, cb) => {
    ...
    async.series([
      cb => {
        // Lookup calendars
        gtfs.getCalendars(agencyKey, timetable.start_date, timetable.end_date, timetable.monday, timetable.tuesday, timetable.wednesday, timetable.thursday, timetable.friday, timetable.saturday, timetable.sunday).then(calendars => {
          if (!calendars.length) {
