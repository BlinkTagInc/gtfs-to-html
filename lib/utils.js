...
/*
 * Get route of next trip with the same block_id
 */
const getTripContinuation = async (trip, timetable) => {
  ...
  // "Continues As" trips must have the same block_id and be a different route_id.
  const tripQuery = {
    agency_key: trip.agency_key,
    block_id: trip.block_id,
    route_id: {$ne: trip.route_id}
  };
  ...
  const lastStoptimeStop = await gtfs.getStops({stop_id: lastStoptimeStopId});
  let firstStopIds;

  if (lastStoptimeStop.length > 0 && lastStoptimeStop[0].parent_station) {
    const parentStationStops = await gtfs.getStops({
      parent_station: lastStoptimeStop[0].parent_station
    }, {stop_id: 1, _id: 0});
    firstStopIds = _.map(parentStationStops, 'stop_id');
  } else {
  ...
  await Promise.all(nextTrips.map(async nextTrip => {
    /*
     * "Continues As" trips must have their first stop_id be the same as the
     * previous trip's last stop_id.
    */
    nextTrip.stoptimes = await gtfs.getStoptimes({
      agency_key: timetable.agency_key,
      trip_id: nextTrip.trip_id,
      stop_id: {$in: firstStopIds}
    }, {}, {lean: true, sort: {stop_sequence: 1}, limit: 1});

    if (nextTrip.stoptimes.length === 0) {
      return;
    }

    const nextTripStoptimeSeconds = formatters.timeToSeconds(nextTrip.stoptimes[0].departure_time);

    /*
     * "Continues As" trips must have their first stop departure time greater
     * than or equal to the previous trip's last stop time arrival time, but not
     * be more than 60 minutes later.
    */
    const maxContinuesAsWaitingTimeSeconds = 60 * 60;
    if (nextTripStoptimeSeconds >= lastStoptimeSeconds && nextTripStoptimeSeconds <= lastStoptimeSeconds + maxContinuesAsWaitingTimeSeconds) {
      if (!soonestTrip || nextTripStoptimeSeconds < formatters.timeToSeconds(soonestTrip.stoptimes[0].departure_time)) {
        soonestTrip = nextTrip;
      }
    }
  }));
