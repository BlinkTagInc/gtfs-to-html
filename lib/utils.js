...
exports.generateHTML = (agencyKey, timetablePage, config) => {
  ...
  return Promise.all(timetablePage.timetables.map(timetable => {
    ...
    .then(() => {
      return gtfs.getTripsByRouteAndDirection(agencyKey, timetable.route_id, timetable.direction_id, timetable.serviceIds);
    })
    .then(trips => {
      if (!trips || trips.length === 0) {
    ...
    .then(() => {
      return getSpecialDates(timetable)
      .then(specialDates => {
    ...
    .then(() => {
      if (config.showMap) {
        let shapesGeojson;
        return gtfs.getShapesByRouteAsGeoJSON(agencyKey, timetable.route_id, timetable.direction_id)
        .then(geojson => {
          shapesGeojson = geojson;
          return gtfs.getStopsByRouteAsGeoJSON(agencyKey, timetable.route_id, timetable.direction_id);
        })
        .then(stopsGeojson => geojsonMerge.merge([shapesGeojson, stopsGeojson]))
        .then(geojson => simplifyGeoJSON(geojson, config.coordinatePrecision))
        .then(geojson => {
          timetable.geojson = geojson;
...
exports.getTimetablePage = (agencyKey, timetablePageId) => {
  ...
  .then(timetables => {
    ...
    .then(result => {
      ...
      timetablePage.timetables = filterAndSortTimetables(timetables, timetablePageId);
      return timetablePage;
    })
    .then(timetablePage => formatTimetablePage(agencyKey, timetablePage));
  });
};
