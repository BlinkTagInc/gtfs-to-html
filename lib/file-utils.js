...
function getTemplatePath(templateFileName, config) {
  const folderPath = config.templatePath === undefined ? path.join(__dirname, '..', 'views/default/') : path.join(untildify(config.templatePath));
 ...
 */
export async function copyStaticAssets(exportPath) {
  const staticAssetPath = path.join(__dirname, '..', 'public');
  await copyFile(path.join(staticAssetPath, 'css'), path.join(exportPath, 'css'));
  await copyFile(path.join(staticAssetPath, 'js'), path.join(exportPath, 'js'));
}
