...
exports.formatGTFSDate = date => moment(date, 'YYYYMMDD');

exports.formatStopTime = (stoptime, config) => {
  if (stoptime) {
    stoptime.classes = [];
  } else {
    stoptime = {
    ...
    config.noServiceSymbolUsed = true;
  }
  ...
  if (stoptime.departure_time === '') {
    ...
    config.requestStopSymbolUsed = true;
