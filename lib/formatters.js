...
exports.formatTimetableLabel = timetable => {
  if (timetable.timetable_label !== '' && timetable.timetable_label !== null && timetable.timetable_label !== undefined) {
    return timetable.timetable_label;
  }

  let timetableLabel = 'Route ';
  if (timetable.routes[0].route_short_name !== '' && timetable.routes[0].route_short_name !== null) {
    timetableLabel += timetable.routes[0].route_short_name;
  } else if (timetable.routes[0].route_long_name !== '' && timetable.routes[0].route_long_name !== null) {
  ...
  if (timetable.stops && timetable.stops.length > 0) {
    ...
    if (firstStop === lastStop) {
      if (timetable.routes[0].route_long_name !== '' && timetable.routes[0].route_long_name !== null) {
        timetableLabel += ` - ${timetable.routes[0].route_long_name}`;
      }
