  ...
  return Promise.all(config.agencies.map(async agency => {
    ...
    for (const [idx, timetablePage] of timetablePages.entries()) {
      ...
      if (timetablePage.timetables.length === 0) {
        logUtils.warn(`${agencyKey}: No timetables found for timetable_page_id=${timetablePage.timetable_page_id}`);
        return;
      }
      ...
      formattedTimetablePage.relativePath = path.join(datePath, sanitize(timetablePage.filename));
