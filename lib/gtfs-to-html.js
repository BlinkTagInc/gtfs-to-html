...
module.exports = config => {
  ...
  return Promise.all(config.agencies.map(async agency => {
    ...
    await fileUtils.prepDirectory(exportPath, assetPath);

    const timetablePages = await utils.getTimetablePages(agencyKey);

    for (const [idx, timetablePage] of timetablePages.entries()) {
      log(`${agencyKey}: Generating ${idx} of ${timetablePages.length} HTML schedules\r`, true);
      ...
      await fileUtils.writeFile(path.join(exportPath, datePath), timetablePage.filename, results.html);
    }

    const logText = await utils.generateLogText(agency, outputStats);

    await fileUtils.writeFile(exportPath, 'log.txt', logText.join('\n'));
