...
const gtfsToHtml = async initialConfig => {
  ...
  if (config.noHead !== true) {
    await copyStaticAssets(exportPath);
  }

  const bar = progressBar(`${agencyKey}: Generating ${config.outputFormat.toUpperCase()} timetables [:bar] :current/:total `, {
    ...
    try {
      ...
      for (const timetable of timetablePage.timetables) {
        for (const warning of timetable.warnings) {
          ...
          bar.interrupt(formatWarning(warning));
        }
      }
      ...
      bar.interrupt(formatWarning(error.message));
    }
