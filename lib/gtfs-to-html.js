...
module.exports = config => {
  ...
  return Promise.all(config.agencies.map(async agency => {
    ...
    log(`${agencyKey}: Creating HTML timetables`);

    await fileUtils.prepDirectory(exportPath);

    const timetablePages = await utils.getFormattedTimetablePages(agencyKey, config);

    const bar = new ProgressBar(`${agencyKey}: Generating HTML timetables [:bar] :current/:total`, {total: timetablePages.length});
    ...
    log(`${agencyKey}: HTML timetables created at ${fullExportPath}`);
    ...
    log(`${agencyKey}: HTML timetable generation required ${seconds} seconds`);
  }));
};
